...
const CommentItem = ({ item, movieId, listComment }) => {
  ...
  useEffect(() => {
    ...
    const reactionApi = item?.reactions.find((item) => item.userId === user?.uid);
  ...
  return (
    <div style={{ marginBottom: '30px' }}>
      <div className="show-comment-items">
        ...
        <div>
          <div className="show-comment-info">
            ...
            <Reaction comment={item} showReaction={showReaction} setShowReaction={setShowReaction} />
            {item?.reactions?.length > 0 && <ShowReaction reactions={item.reactions} />}
          </div>
          {user && (
            <div className="comment-action">
              <p
                ...
                style={{
                  color: reaction?.type === 'like' ? '#243FD3' : reaction?.type === 'love' ? '#F33E58' : '#EAB125',
                }}
              >
                {showReaction ? (
              ...
              <p onClick={() => setShowReply(!showReply)}>{showReply ? 'Cancel' : 'Reply'}</p>
            </div>
          )}
        </div>
      </div>

      {showReply && <InputReply user={user} commentParentId={item.id} movieId={movieId} setShowReply={setShowReply} />}
      ...
      {showReplyItem && <ReplyCommentList listComment={listComment} commentParentId={item.id} movieId={movieId} />}
    </div>
  );
};
