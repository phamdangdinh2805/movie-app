...
function ModalTrailer({ show, setShow }) {
  ...
  return (
    <div className="overlay" style={{ display: show ? 'flex' : 'none' }} onClick={() => setShow(false)}>
      <div className="trailer-container" onClick={(e) => e.stopPropagation()}>
        <div className="trailer-title-close">
          <h1 className="trailer-title">Movies Trailer</h1>
          <box-icon onClick={() => setShow(false)} color="white" size="md" name="x-circle"></box-icon>
        </div>
