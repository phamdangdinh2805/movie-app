...
import Simular from '../../components/Similar/Similar';
            ...
            <div className="details-info">
              ...
              {loading ? (
                ...
                <p className="release_date">
                  {data.release_date ? `Release date: ${data.release_date}` : `Last episode: ${data.last_air_date}`}
                </p>
              )}
              <div className="genres">
                {data.genres && data.genres.map((item) => <Button key={item.id} content={item.name} />)}
              </div>
              <div className="ratings">
                <StarRatings
                  rating={data.vote_average}
                  starRatedColor="#feb041"
                  numberOfStars={10}
                  name="rating"
                  starDimension="15px"
                  starSpacing="2px"
              ...
              <div className="watch">
                <Link
                  ...
                  to={media_type === 'tv' ? `/watch/tv/${id}/season/1/esp/1` : `/watch/movie/${id}`}
                >
                  Watch Now
                </Link>
                ...
                <span className="favorite-container" onClick={handleAddToFavorites}>
                  <MdFavoriteBorder className="favorite-border"/>
                </span>
              </div>
            </div>
          </div>
